...
export default function ParentDashboard({ parentId, accessCode }: ParentDashboardProps) {
  ...
  const addChild = async () => {
    ...
    // Определяем возраст и режим просмотра
    const age = typeof newChildAge === 'number' ? newChildAge : null
    const defaultViewMode = age !== null && age <= 9 ? 'balloons' : 'list'
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-3 pt-3 border-t border-gray-200">
                        <div className="text-xs text-gray-600 mb-2">Режим просмотра по умолчанию:</div>
                        <div className="flex gap-2">
                          <button
                            onClick={async (e) => {
                              ...
                              await supabase
                                .from('children')
                                .update({ default_view_mode: 'list' })
                                .eq('id', child.id)
                              loadDashboardData()
                            }}
                            className={`flex-1 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                              child.default_view_mode === 'list'
                          ...
                          <button
                            onClick={async (e) => {
                              ...
                              await supabase
                                .from('children')
                                .update({ default_view_mode: 'balloons' })
                                .eq('id', child.id)
                              loadDashboardData()
                            }}
                            className={`flex-1 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                              child.default_view_mode === 'balloons'
            ...
            <div className="space-y-4 mb-6">
              ...
              <div>
                ...
                <input
                  ...
                  value={newChildAge}
                  onChange={(e) => setNewChildAge(e.target.value ? parseInt(e.target.value) : '')}
                  min="3"
                  max="18"
                  className="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:outline-none focus:border-purple-500"
                />
                <p className="text-xs text-gray-500 mt-2">
                  💡 До 9 лет по умолчанию шарики 🎈, от 10 лет - список 📋
                </p>
              </div>
            </div>
